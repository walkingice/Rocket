        ...
        targetView?.postDelayed(
            {
                if (fragment.isResumed && parentView != null) {
                    ...
                    val view = LayoutInflater.from(fragment.activity)
                        .inflate(R.layout.download_indicator_intro, parentView)
                    ...
                    menuParams.setMargins(
                        0,
                        0,
                        metrics.widthPixels - menuParams.width - location[0],
                        metrics.heightPixels - menuParams.height - location[1]
                    )

                    // Adjust pointer position
                    val pointer =
                        view.findViewById<ImageView>(R.id.download_indicator_intro_pointer)
                    val pointParams = pointer.layoutParams as ConstraintLayout.LayoutParams
                    pointParams.setMargins(
                        0,
                        0,
                        metrics.widthPixels - menuParams.width / 2 - pointParams.width / 2 - location[0],
                        0
                    )
